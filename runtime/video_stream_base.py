...
from ._compat import init_logger

from . import video_stream_envs
from .video_frame_filter import FrameSimilarityFilter
from .video_stream_context import (
    text_only_message,
)
from ._compat import OmniRequestOutput
...
class VideoStreamPipelineHooks(Protocol):
    ...
    def build_engine_prompt(
        ...
        [本仓库分叉] 上游不传 frame_metadata；我们加上它（带默认值，不破坏上游签名），
        因为要给每帧配 ``<X.X seconds>`` 时间戳，而 pts_ms 只在 metadata 里。
        """
        ...
...
class StreamingVideoSessionConfig(BaseModel):
    ...
    enable_frame_filter: bool = Field(
        default=False,
        description=(
            "EVS pixel-similarity pre-filter to drop near-duplicate frames. "
            "[本仓库分叉] 上游默认 True，我们改成 False —— 见下方说明，接口保留。"
        ),
    )
...
class OmniStreamingVideoHandler:
    ...
    def on_frame_buffered(
        ...
        [本仓库分叉] 多传这一帧的 metadata（pts_ms 等），决策层要按它对齐事实源。"""
        del raw_bytes, frame_b64, message_history, config, frame_meta
            ...
            async def _processor() -> None:
                ...
                while True:
                    ...
                    if msg_type == "_internal.frame_decode_failed":
                        ...
                        self.on_frame_buffered(raw_bytes, frame_data, message_history, config, frame_metadata[-1])
                        await self._send_frame_ack(
                            websocket,
                            msg,
                            accepted=True,
    ...
    async def _process_query_engine(
        ...
        messages, user_message = self.build_engine_prompt(
            ...
            query_text,
            prewarmed_frames,
            frame_metadata=frame_metadata,   # [本仓库分叉] 让 hook 拿得到每帧 pts_ms
        )
